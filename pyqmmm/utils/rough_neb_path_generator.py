...
def user_input():
    ...
    temp = [
        (lambda sub: range(sub[0], sub[-1] + 1))(list(map(int, ele.split("-"))))
        for ele in coord2_input.split(",")
    ]
    coord2 = [b for a in temp for b in a]
    # How many images should be in the NEB path
    image_count = (
        int(input("Into how many bins would you like to divide the data?")) + 1
    )
...
def get_final_master(frame_indices, master_list, ts_dict):
    ...
    for i, index in enumerate(frame_indices):
        ...
        if i + 1 > len(frame_indices) - 1:
            continue
        if (
            master_list[index]["dist_diff"] < ts_dict["dist_diff"]
            and master_list[frame_indices[i + 1]]["dist_diff"] > ts_dict["dist_diff"]
        ):
...
def rough_neb_path_generator():
    ...
    image_count = 20
    print(
        "Step 1: Your atom indices are {} and {} and you want {} frames.".format(
    ...
    print(
        "Step 4: Your max value is {} and your min value is {}.".format(
