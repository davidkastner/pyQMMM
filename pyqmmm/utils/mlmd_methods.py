...
def data_processing(df, labels):
    ...
    scaler = MinMaxScaler()
    df_norm = pd.DataFrame(scaler.fit_transform(df.values),
                           columns=df.columns,
                           index=df.index)
...
def run_trp_cage():
    ...
    feature_extractors = [
        ...
        fe.MlpAeFeatureExtractor(activation=relprop.relu, classifier_kwargs={
                                 'solver': 'adam', 'hidden_layer_sizes': (100,)}, **kwargs),
        fe.RandomForestFeatureExtractor(one_vs_rest=True, classifier_kwargs={
                                        'n_estimators': 100}, **kwargs),
        fe.KLFeatureExtractor(**kwargs),
        fe.MlpFeatureExtractor(classifier_kwargs={'hidden_layer_sizes': (
            120,), 'solver': 'adam', 'max_iter': 1000000}, activation=relprop.relu, **kwargs),
    ]
