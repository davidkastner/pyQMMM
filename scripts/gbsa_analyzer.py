...
def update_res_names(df):
    ...
    res_names = {'AG2':'ARG', 'AN1':'ASN', 'HIE':'HIS', 'TR1':'TYR', 'HD1':'HIS', 'HD2':'HIS', 'AP1':'ASP', 'CL1':'CL', "SC1": "SUC"}
...
def plot_single_all_gbsa(df, file_name):
    ...
    ax = df.plot.bar(x='Residue', y=['VDW','Electrostatic','Polar','Non-polar'], color = colors)
    figure_formatting()
    ax.set_ylabel("GBSA energy score", weight='bold')
    ax.set_xlabel("Residue", weight='bold')
    plt.savefig(file_name, bbox_inches='tight', transparent=True)
...
def plot_clustered_stacked(dataframes, labels, y_columns, sorted_x_labels, file_name):
    H="//"
    plt.axhline(y=0, color='k', alpha = .5, linestyle='-', linewidth=3)
    colors = ['#fb8500', '#ffb703', '#023047', '#219ebc', '#8ecae6']
    ...
    handles,axe_labels = axe.get_legend_handles_labels() # get the handles we want to modify
    for i in range(0, number_of_df * number_of_col, number_of_col): # len(h) = n_col * n_df
        sliced_handles = handles[i:i+number_of_col] if i == 0 else handles[i-1:i+number_of_col]
    ...
    l1 = axe.legend(handles[:number_of_col], list(axe_labels[:number_of_col-1]), loc=[1.01, 0.5])
    if labels is not None:
        l2 = plt.legend(n, labels, loc=[1.01, 0.1]) 
    axe.add_artist(l1)
    axe.set_ylabel("GBSA energy score", weight='bold')
    axe.set_xlabel("Residue", weight='bold')
    axe.set_xticks([tick + .65 for tick in axe.get_xticks()])
    plt.savefig(file_name, bbox_inches='tight', transparent=True)
    return axe

def plot_multi_all_gbsa(df_hits_list, df_list, y_columns, sorted_x_labels):
    acute_df_hits, obtuse_df_hits, series_columns  = prep_multi_gbsa_data(df_hits_list, df_list, y_columns)
    plot_clustered_stacked([acute_df_hits[series_columns], obtuse_df_hits[series_columns]], ["acute","obtuse"], y_columns, sorted_x_labels, "test.pdf")
...
def prep_multi_gbsa_data(df_hits_list, df_list, y_columns):
    ...
    for residue in residues:
        if residue not in acute_residues and residue in obtuse_residues:
          residue_index = obtuse_df_hits.loc[obtuse_df_hits["Residue"] == residue]["index"].tolist()[0]
          ...
          residue_index = acute_df_hits.loc[acute_df_hits["Residue"] == residue]["index"].tolist()[0]
